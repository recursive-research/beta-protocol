...
async function main() {
  ...
  const tokenTradeAmount = ethers.utils.parseEther('200000');
  ...
  await getTokens(rando, token, tokenTradeAmount);

  await router
    .connect(rando)
    .swapExactTokensForTokens(tokenTradeAmount, 0, [token.address, weth.address], rando.address, 2000000000);
