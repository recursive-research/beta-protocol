...
async function main() {
  const poolAddress = Deployments.mainnet.prqPool;
  const tokenDepositAmount = ethers.utils.parseEther('1000000');
  const wethTradeAmount = ethers.utils.parseEther('1000'); // ~$5mm
  ...
  console.log('---              fixed rate:', (await pool.fixedRate()).toString());
  ...
  await token.connect(rando).approve(router.address, ethers.constants.MaxUint256);
  ...
  for (let i = 0; i < 30; i++) {
    if (i % 2 == 0) {
      ...
      await router
        .connect(rando)
        .swapExactTokensForTokens(randoBalance, 0, [weth.address, token.address], rando.address, 2000000000);
    } else {
      const randoBalance = await token.balanceOf(rando.address);
      await router
        .connect(rando)
        .swapExactTokensForTokens(randoBalance, 0, [token.address, weth.address], rando.address, 2000000000);
    }
  }
