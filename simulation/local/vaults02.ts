...
// This is the scenario that governance tokens crash in value compared to ETH
...
async function main() {
  ...
  // Swap $50m of ALCX for ETH
  const alcxTradeAmount = ethers.utils.parseEther('100000'); // ~$50mm
  await getTokens(rando, alcx, alcxTradeAmount);
  await sushiRouter
    .connect(rando)
    .swapExactTokensForTokens(alcxTradeAmount, 0, [alcx.address, weth.address], rando.address, 2000000000);

  // Swap $400k INJ for ETH (almost the entire pool)
  const injTradeAmount = ethers.utils.parseEther('30000'); // ~400k
  await getTokens(rando, inj, injTradeAmount);
  await sushiRouter
    .connect(rando)
    .swapExactTokensForTokens(injTradeAmount, 0, [inj.address, weth.address], rando.address, 2000000000);

  // swap $12m FTM for ETH
  const ftmTradeAmount = ethers.utils.parseEther('4000000'); // ~$12m
  await getTokens(rando, ftm, ftmTradeAmount);
  await sushiRouter
    .connect(rando)
    .swapExactTokensForTokens(ftmTradeAmount, 0, [ftm.address, weth.address], rando.address, 2000000000);
